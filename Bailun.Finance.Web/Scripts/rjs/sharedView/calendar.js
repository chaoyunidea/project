...
define(function () {
    ...
    function renderCalendarList(data) {
        ...
        for(var i=0;i<data.length;i++){
            str += '<li><a href="/calendar/unscramble/' 
            + data[i].SourceId + "_" + (data[i].UnscrambleId || '0') + "_" + data[i].ReleasedDate + '" target="_blank">'
            + '<div class="time clearfix">'
            + ' <em>' + new Date(data[i].ReleasedDate).pattern("HH:mm") + '</em>'
            + '<div class="star-union">'
            + renderStar(data[i].Star)               
            + '</div>'
            + '</div>'
            + '<div class="title clearfix">'                                
            + '<img src="/images/banner/rectangle/' + data[i].CountryImg + '" class="flag" />'
            + '<em class="nation">' + data[i].Country + '</em>'                                
            + ' <em class="title-con">' + data[i].Title + '</em>'
            + '</div>'
            + '<div class="param clearfix">'
            + '<p>今值: <em>' + data[i].Actual + '</em></p>'
            + '<p>预测: <em>' + data[i].Consensus + '</em></p>'
            + '<p>前值: <em>' + (data[i].Revised == "---" ? data[i].Previous : data[i].Revised) + '</em></p>'
            + '</div></a></li>';  
        }
