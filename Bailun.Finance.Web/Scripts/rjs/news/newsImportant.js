define([js.calculatePrice],function (calc) {
    ...
    Nav = {
        //标题
        titlelist: function () { return $('.nav').find('.tab-title').find('div') },
        ListT: function (i) { return $('.nav').find('.tab-content').children('[data-index="' + i + '"]'); },
    ...
    function setData(index, data) {
        ...
        if (data.length > 0) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                newsAreaStr += '<div class="nav-data clearfix ' + priceStatus(calcVal.priceExchange) + (index == 9 ? ' press' : '') + '" data-code="' + data[i].Code + '" data-timezone="' + data[i].Price_TimeZone + '"><a href="' + priceDomain + '/marketprice/detail/' + data[i].MarketId + '" target="_blank"><div class="data-contain clearfix"><p class="title">'
                            + data[i].Cn_Name 
                            + '</p><p class="current-price">' + data[i].CurrentPrice
                            + '</p></div>' + (index == 9 ? ('<div class="exchange">' + data[i].Exchange + '</div>') : '')
                            + '<div class="data-left"><span class="open-price">' + calcVal.priceExchange
                            + '</span><span class="close-price">(' + calcVal.priceLimit
                            + ')</span><p class="data-time">' + (data[i].PriceDate ==""?"---":new Date(transTimezone(data[i].PriceDate,data[i].Price_TimeZone)).pattern('HH:mm'))
                            + '</p></div><div class="chart" id="' +  (index == 0 ? 'chartLineMain' : 'chartLine') + data[i].MarketId + '" style="width:55px;height:50px;"></div></a></div>';
            }
    ...
    refreshPriceData = function(data) {
        ...
        str += '<div class="data-left"><span class="open-price">' + calcVal.priceExchange
            + '</span><span class="close-price">(' + calcVal.priceLimit
            + ')</span><p class="data-time">' + (data.PriceDate == "" ? "---" : new Date(transTimezone(data.PriceDate,$('.nav-data[data-code="' + data.Code + '"]').attr('data-timezone'))).pattern('HH:mm'))
            + '</p></div>';
