...
require([js.price, js.newsHead, js.calendar], function (priceBox, newsHead, calendar) {
    ...
    var Dom = {
        //标题
        titlelist: function () { return $title.find('div') },

        ListT: function (i) { return $content.children('[data-index="' + i + '"]'); }
    };
    ...
    var _HTML = {
        ...
        updateNews: function (data, type) {
            ...
            //新增
            if (instantDate.getDate() != new Date(parseInt(currentItem.find('.news-list li').first().attr('data-date'))).getDate()) {//判断是否是当天数据
                if (data.ReleasedDate > parseInt(currentItem.find('.news-list li').first().attr('data-date'))) {
                    instantStr += '<div class="time"><p>'
                                + (instantDate.getMonth() + 1) + '月' + instantDate.getDate() + '日' + '</p></div>';
                    ...
                    currentItem.children().first().find('.news-list').prepend(instantStr)
                        .find('.time:first').prepend(clockEle);
                }
            } else {
                instantStr = this.listItemRender(data, isFast);
                if (data.ReleasedDate < parseInt(currentItem.find('.news-list li').first().attr('data-date'))) {//推送新闻时间比第一条还早
        ...
        setData: function (index, data, initDate) {
            ...
            for (var i = 0; i < data.length; i++) {
                if (new Date(data[i].ReleasedDate).getDate() != currentDate.getDate()) {
                    ...
                    newsAreaStr += '<div class="time"><p>'
                                + (currentDate.getMonth() + 1) + '月' + currentDate.getDate() + '日' + '</p></div>';
                }
        ...
        listItemRender: function (obj, isFresh) {
            ...
            switch (obj.Type) {
                case 0://新闻
                    newsListStr += '<li class="level' + obj.Level + (isFresh ? ' fresh' : '') + '" data-date="' + obj.ReleasedDate + '" data-id="' + obj.SourceId + '"><div class="post-time"><i></i><em>'
                                + ge_time_format(obj.ReleasedDate, 3)
                                + '</em></div><div class="content"><div class="info">' + obj.Content.replace(/\r\n/g, '<br>') + '</div>'
                                + '<div class="share"><span class="share-icon iconfont">&#xe60f;</span><div class="share-box"><i class="shape"></i>'
                                + '<ul class="bdsharebuttonbox "><li><a href="javascript:;" data-bshare=\"{type:\'weixin\',url:\'\',title:\'\',desc:\'' + (obj.Title.replace(/<br \/><p ><p \/>/g, '。')).replace(/\s/g, '') + '\',summary:\'\',images:\'' + window.location.origin + '/images/public/push.png\'}\" class="weixin">'
                                + '<i class="iconfont">&#xe50a;</i>微信</a></li>'
                                + '<li><a href="javascript:;" data-bshare=\"{type:\'weibo\',url:\'\',title:\'\',desc:\'' + (obj.Title.replace(/<br \/><p ><p \/>/g, '。')).replace(/\s/g, '') + '\',summary:\'\',images:\'' + window.location.origin + '/images/public/push.png\'}\" class="weibo"><i class="iconfont"}\" >&#xe504;</i>微博</a></li>'
                                + '<li><a href="javascript:;" data-bshare=\"{type:\'qzone\',url:\'\',title:\'\',desc:\'' + (obj.Title.replace(/<br \/><p ><p \/>/g, '。')).replace(/\s/g, '') + '\',summary:\'\',images:\'' + window.location.origin + '/images/public/push.png\'}\" class="qq"><i class="iconfont">&#xe64e;</i>QQ</a></li></ul></div></div>';
                    break;
                case 1://日历
                    newsListStr += '<li class="calendar level' + obj.Level + (isFresh ? ' fresh' : '') + '" data-date="' + obj.ReleasedDate + '" data-id="' + obj.SourceId + '"><div class="post-time"><i></i><em>'
                        + ge_time_format(obj.ReleasedDate, 3)
                        + '</em></div><div class="content"><dl><dd class="flag"><img src="/Images/banner/rectangle/'
                        + obj.CountryImg + '" onerror="javascript:this.src=\'/images/banner/rectangle/default_country.png\'"/></dd><dd class="act-time">'
                        + new Date(obj.AddDate).pattern('HH:mm') + '</dd><dd class="title"><p class="name">'
                        + obj.Title + '</p><p class="value">前值：'
                        + obj.Previous + ' 预期：'
                        + obj.Consensus + '</p>' + (obj.Revised != null && obj.Revised != '---' ? ('<p class="revised">修正：' + obj.Revised + '（前值）</p>') : '') + '</dd><dd class="star"><div class="star-level ' + (obj.Star > 2 ? '' : 'low') + '"><i style="width:'
                        + obj.Star * 20 + '%"></i></div></dd><dd class="act-val">公布：'
                        + obj.Actual + '</dd><dd class="impact"><span class="impact' + obj.Impact + '">'
                        + impactArr[obj.Impact] + '</span></dd></dl>'
                        + '<div class="share"><span class="share-icon iconfont">&#xe60f;</span><div class="share-box"><i class="shape"></i>'
                        + '<ul class="bdsharebuttonbox "><li><a href="javascript:;" data-bshare=\"{type:\'weixin\',url:\'\',title:\'\',desc:\'' + (obj.Title.replace(/<br \/><p ><p \/>/g, '。')).replace(/\s/g, '') + '\',summary:\'\',images:\'' + window.location.origin + '/images/public/push.png\'}\" class="weixin">'
                        + '<i class="iconfont" >&#xe50a;</i>微信</a></li><li><a href="javascript:;" data-bshare=\"{type:\'weibo\',url:\'\',title:\'\',desc:\'' + (obj.Title.replace(/<br \/><p ><p \/>/g, '。')).replace(/\s/g, '') + '\',summary:\'\',images:\'' + window.location.origin + '/images/public/push.png\'}\" class="weibo">'
                        + '<i class="iconfont" >&#xe504;</i>微博</a></li><li><a href="javascript:;" data-bshare=\"{type:\'qzone\',url:\'\',title:\'\',desc:\'' + (obj.Title.replace(/<br \/><p ><p \/>/g, '。')).replace(/\s/g, '') + '\',summary:\'\',images:\'' + window.location.origin + '/images/public/push.png\'}\" class="qq">'
                        + '<i class="iconfont" >&#xe64e;</i>QQ</a></li></ul></div></div>';
                    break;
            }
        ...
        sort: function (data) {
            return data.sort(function (a, b) { return a.index - b.index });
        },
    ...
    function newsTab(obj) {
        if (!obj.hasClass('active')) {//切换新闻平台
            obj.addClass('active').siblings().removeClass('active');
            $('.news-content').children('[data-index="' + obj.attr('data-index') + '"]').addClass('active')
    ...
    function noticeCheckFn(obj) {
        if (obj.children('span').hasClass('checked')) {//取消选择
    ...
    function noticeOnOffInit() {
        ...
        tipsOnOff.noticeSelected = msgCookie ? msgCookie.split(',').allMap(function (i) { return parseInt(i)}) : [];
        tipsOnOff.soundSelected = ringCookie ? ringCookie.split(',').allMap(function (i) { return parseInt(i) }) : [];
    ...
    function setStatusCookie(type, status) {//设置cookie
