require([js.newss, js.controlBar], function (news, controlBar) {
    ...
    $('#loading').openMask({ image: '/Images/1.0.0/Icon/loading.gif', isMask: false, Method: function () { $('#loading').height(600); } });
    ...
    function searchListRender(index,data) {
        var str = '';
        str += '<div class="area price">'
            + '<div class="heading clearfix">'
            + '<h3>' + productJson[index][0] + '</h3>'
        ...
        for (var i = 0; i < data.length; i++) {
            str += '<li>'
                + '<a href="/marketprice/' + (5 < index && index < 9 ? 'stockdetail' : 'detail') + '/' + data[i].MarketId + '" target="_blank" class="clearfix">'
                + '<h4 class="name">' + data[i].Cn_Name + '</h4>'
                + '<p class="code">代码 <em>' + data[i].Code + '</em></p>'
                + '</a>'
                + '</li>';
        }
