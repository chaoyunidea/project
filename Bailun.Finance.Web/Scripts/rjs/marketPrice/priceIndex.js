require([js.newss, js.controlBar, js.calculatePrice,js.calendar], function (news, controlBar,calc,calendar) {
    ...
    refreshPriceData = function(data){
        ...
        str += '<div class="data-area"><h3 class="current">' + data.CurrentPrice + '<i></i></h3>'
            + '<p class="updowns"><em class="percent">' +calcVal.priceExchange  + '</em><em>'
            + calcVal.priceLimit + '</em></p></div>';
    ...
    function priceListRenderDetail(data,type) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            str += '<li data-code="' + data[i].Code + '" class="' + priceStatus(calcVal.priceExchange) + '">'
                + '<div class="params">'
                + '<div class="name clearfix"><h5>' + data[i].Cn_Name + '</h5>'
                + (type == 9 ? ('<h6>' + data[i].Exchange + '</h6>') : '') + '</div>'
                + '<div class="data-area"><h3 class="current">' + parseFloat(data[i].CurrentPrice) + '<i></i></h3>'
                + '<p class="updowns"><em class="percent">' + calcVal.priceExchange + '</em><em>'
                + calcVal.priceLimit + '</em></p></div>'
                + '</div>'
                + '<div class="chart" id="chartLine' + data[i].MarketId + '">'
                + '</div>'
                + '<a href="/marketprice/detail/' + data[i].MarketId + '" target="_blank">'
                + '</a>'
                +  '</li>';
        }
        return str;
    ...
    function priceListRenderStock(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            str += '<li>'
                + '<a href="/marketprice/stockdetail/' + data[i].MarketId + '" target="_blank">'
                + '<div class="params ' + (data[i].CurrentPrice - data[i].OpenPrice > 0 ? 'up' : 'down') + '">'
                + '<h5 class="name">' + data[i].Cn_Name + '</h5>'
                + '<h3 class="current">' + toFixeds(parseFloat(data[i].CurrentPrice), 2) + '<i></i></h3>'
                + '<p class="updowns"><em class="percent">' + (data[i].OpenPrice <= 0 ? toFixeds(parseFloat(data[i].OpenPrice), 2) : toFixeds((parseFloat(data[i].CurrentPrice - data[i].OpenPrice) / data[i].OpenPrice * 100), 2)) + '%</em><em>'
                + toFixeds(parseFloat((data[i].CurrentPrice - data[i].OpenPrice)), 2) + '</em></p>'
                + '</div>'
                + '<div class="chart" id="chartLine' + data[i].MarketId + '">'
                + '</div>'
                + '</a>'
