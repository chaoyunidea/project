...
function parseJSONorNot(mayBeJSON) {
    if (typeof mayBeJSON === 'string') {
        try {
            return JSON.parse(JSON.parse(mayBeJSON))
        }
        catch (e) {
...
var Datafeeds = {}, windowRangTime = [0, 0]; //0是倒数第二条  1是最后一条
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function (symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
    ...
    if (WindowCallback) { huaxian(WindowCallback); return; }
	...
	.done(function (response) {
	    var lastTime = rangeEndDate;
	    var ThatTime = parseInt(new Date().getTime().toString().slice(0,-3));
        



	    try {
	        var response = parseJSONorNot(JSON.parse(response));

	        var t = response.t;
	        if (t.length > 1) {
	            windowRangTime[0] = Math.max(windowRangTime[0], t[t.length - 2]);
	            windowRangTime[1] = Math.max(windowRangTime[1], t[t.length - 1]);
	        } else if (t.length == 1) {
	            windowRangTime[1] = Math.max(windowRangTime[1], t[t.length - 1]);
	        }
	        var gmt = 8 - parseInt(response.gmt);

	        for (var i = 0; i < t.length; i++) {
	            response.t[i] = parseInt(response.t[i]) + (gmt * 60 * 60)
	        }

           // 当前的时间 - 取值的最大时间
	        if (ThatTime - lastTime > 60) { //取历史记录的时候  去掉最后一条数据  最后一条是最近的动态数据
	        ...
	        huaxian(JSON.stringify(response), rangeStartDate);
    ...
    function huaxian(response, date) {
        var data = parseJSONorNot(response);
...
Datafeeds.UDFCompatibleDatafeed.prototype.subscribeBars = function (symbolInfo, resolution, onRealtimeCallback, listenerGUID, onResetCacheNeededCallback) {
    ...
    if (tvWidget.ParentWindowCallback.Varieties != symbolInfo.name || tvWidget.ParentWindowCallback.resolution != resolution) {
        tvWidget.ParentWindowCallback.Ticker = symbolInfo.ticker;
        tvWidget.ParentWindowCallback.Varieties = symbolInfo.name;
        tvWidget.ParentWindowCallback.resolution = resolution;
        tvWidget.ParentWindowCallback.Callback(symbolInfo.name, resolution, symbolInfo.ticker);
    }
};
