$(function () {
    ...
    function economicListRender(data) {
        ...
        if (data.length > 0) {
            for (var i = 0; i < data.length; i++) {
                str += '<tr>' + (data[i].showSpan ? '<td class="time" rowspan="' + data[i].RowSpan + '">' + ge_time_format(data[i].AddDate, 3).slice(0, 5) + '</td>' : '') + (data[i].showSpan ? '<td class="ensign" rowspan="' + data[i].RowSpan + '"><img src="/images/banner/square/' + data[i].CountryImg + '" onerror="javascript:this.src=\'/images/banner/square/default_country.png\'"/></td>' : '') + '<td align="left">' + data[i].Title + '</td>' + '<td>' + '<div class="star-level ' + (data[i].Star > 2 ? '' : 'low') + '">' + '<i style="width:' + data[i].Star * 20 + '%"></i>' + '</div>' + '</td>' + (data[i].Revised == '---' || data[i].Revised == 0 ? '<td>' + data[i].Previous + '</td>' : '') + (data[i].Revised != '---' && data[i].Revised != 0 ? '<td><div class="front-val">' + data[i].Revised + '<span>修正前 <br />' + data[i].Previous + '</span></div></td>' : '') + '<td>' + data[i].Consensus + '</td>' + '<td class="highlight">' + (data[i].Actual == '---' ? '未公布' : data[i].Actual) + '</td>' + '<td class="trend ' + (data[i].Star > 2 ? 'greater' : '') + '">' + '<span class="' + eco.color[data[i].Impact] + '">'
                // 没修改前
                //+ '<em>' + eco.impact[data[i].Impact] + '</em>'
                + eco.impact[data[i].Impact] + '</span>' + '</td>' + '<td class="detail">' + '<a href=' + (data[i].UnscrambleId == null ? '"javascript:;' : '"/calendar/unscramble/' + data[i].SourceId + '_' + data[i].UnscrambleId + '_' + data[i].ReleasedDate) + '" target="_blank">' + '<img src="/Images/calendar/detail-link.png" />' + '</a></td></tr>';
            }
        } else {
            str += '<tr>' + '<td colspan="9" class="no-data">' + '<img src="/Images/calendar/no-info.png" />' + '<em>今日无' + ($('.level .item dd:gt(0)').hasClass('active') ? $('.level .item dd.active').text() : '') + '财经数据</em></td></tr>';
        }
        $('#economic').html(str);
    }
    currency();
    ...
    function overallListRender(data) {
        ...
        if (data.length > 0) {
            for (var i = 0; i < data.length; i++) {
                str += '<tr>' + (data[i].IsDetermine ? '<td class="time">待定</td>' : '<td class="time">' + ge_time_format(data[i].AddDate, 3).slice(0, 5) + '</td>') + '<td class="ensign">' + data[i].Country + '</td>' + '<td>' + data[i].City + '</td>' + '<td>' + '<div class="star-level ' + (data[i].Star > 2 ? '' : 'low') + '">' + '<i style="width:' + data[i].Star * 20 + '%"></i>' + '</div>' + '</td>' + '<td align="left" class="content">' + (data[i].PeopleImg.length ? '<img src="//' + data[i].PeopleImg + '"/>' : '') + '<p>' + data[i].EventContent + '</p>' + '</td>' + '</tr>';
            }
        } else {
            str += '<tr>' + '<td colspan="5" class="no-data">' + '<img src="/Images/calendar/no-event.png" />' + '<em>今日无财经大事</em></td></tr>';
        }
    ...
    function holidayListRender(data) {
        ...
        if (data.length > 0) {
            for (var i = 0; i < data.length; i++) {
                str += '<tr>' + '<td class="time">' + new Date(data[i].AddDate).pattern('yyyy-MM-dd') + '</td>' + '<td class="ensign">' + data[i].Country + '</td>' + '<td>' + data[i].Exchange + '</td>' + '<td>' + data[i].HolidayName + '</td>' + '<td>' + data[i].Note + '</td>' + '</tr>';
            }
        } else {
            str += '<tr>' + '<td colspan="5" class="no-data">' + '<img src="/Images/calendar/no-holiday.png" />' + '<em>今日无假期休市安排</em></td></tr>';
        }
