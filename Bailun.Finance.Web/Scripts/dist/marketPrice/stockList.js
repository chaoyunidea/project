require([js.newss, js.controlBar], function (news, controlBar) {
    ...
    function breadRender() {
        var str = '<span><a href="/marketprice/index">市场报价</a> > </span> <span><a href="/marketprice/' + productJson[productId][1] + '">' + productJson[productId][0] + '</a> </span>'
        //+ '<span><a>' + plateTypeArr[parseInt(plateType) + 1] + '</a></span>'
        + '<span class="plate-select"><img src="/Images/marketPrice/slide-down.png" />' + '<em>' + '</em>' + '<div class="set-box">' + '<ul></ul>' + '<b class="shape"></b>' + '</div></span>';
    ...
    function renderPlateData(data) {
        var str = '';
        for (var i = 0; i < data.length; i++) {
            str += '<li>' + '<a href="/marketprice/StockDetail/' + productId + '-' + plateId + '-' + data[i].MarketId + '" target="_blank">' + '<div class="inside clearfix">' + '<div class="f-l">' + '<h2 class="name">' + data[i].Market_CnName + '</h2>' + '<div class="code">' + '<em>' + data[i].Market_Code + '</em>' + '</div>' + '<div class="total ' + (data[i].CurrentPrice - data[i].ClosePrice > 0 ? 'up' : 'down') + '">' + '<i></i>' + '<em>' + toFixeds(parseFloat(data[i].CurrentPrice), 2) + '</em>' + '</div>' + '<span></span>' + '<dl class="param ' + (data[i].CurrentPrice - data[i].ClosePrice > 0 ? 'up' : 'down') + '">' + '<dd><h5>涨跌额</h5><p class="color">' + toFixeds(parseFloat(data[i].CurrentPrice - data[i].ClosePrice), 2) + '</p></dd>' + '<dd><h5>涨跌幅</h5><p class="color">' + (data[i].ClosePrice <= 0 ? toFixeds(parseFloat(data[i].ClosePrice), 2) : toFixeds(parseFloat(data[i].CurrentPrice - data[i].ClosePrice) / data[i].ClosePrice * 100, 2)) + '%</p></dd>' + '<dd><h5>昨收</h5><p>' + toFixeds(parseFloat(data[i].ClosePrice), 2) + '</p></dd>' + '<dd><h5>今开</h5><p>' + toFixeds(parseFloat(data[i].OpenPrice), 2) + '</p></dd>' + '<dd><h5>最高</h5><p>' + toFixeds(parseFloat(data[i].HighPrice), 2) + '</p></dd>' + '<dd><h5>最低</h5><p>' + toFixeds(parseFloat(data[i].LowPrice), 2) + '</p></dd>' + '<dd><h5>成交量/手</h5><p> ' + (data[i].TotalVolume == null ? '---' : parseInt(data[i].TotalVolume)) + '</p></dd>' + '<dd><h5>成交额/万</h5><p>' + (data[i].TotalVolume == null ? '---' : parseInt(data[i].TotalVolumePrice)) + '</p></dd>' + '</dl>' + '</div>' + '<div class="chart" id="chartLine' + data[i].MarketId + '"></div>' + '</div>' + '</a>' + '</li>';
        }
        return str;
    }
