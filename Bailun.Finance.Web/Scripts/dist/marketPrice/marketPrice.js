require([js.newss, js.controlBar, js.calculatePrice, js.calendar], function (news, controlBar, calc, calendar) {
            ...
            plotOptions: {
                area: {
                    fillColor: {
                        ...
                        stops: [[0, Highcharts.Color(options.colors[0]).setOpacity(0.5).get('rgba')], [1, Highcharts.Color(Highcharts.getOptions().colors[0]).setOpacity(0).get('rgba')]]
                    },
                    marker: {
                        radius: 0
                    },
    ...
    function loopRender(data) {
        var str = '';
        switch (category) {
            case 'indices':case 'bonds':case 'stockIndex':case 'goods':
                //股指
    ...
    function refreshListRender(data) {
        ...
        str += '<div class="data-area">' + '<div class="total ' + priceStatus(calcVal.priceExchange) + '">' + '<div class="big"><em>' + data.CurrentPrice + '</em>' + '</div><div class="small"><i></i><em>' + calcVal.priceExchange + '</em><em>(' + calcVal.priceLimit + ')</em></div></div>' + '<dl class="param">' + (category == 'forex' || category == 'metals' ? '<dd><h5>昨收</h5><p>' + data.ClosePrice + '</p></dd>' + '<dd><h5>今开</h5><p>' + data.OpenPrice + '</p></dd>' : '') + '<dd><h5>最高</h5><p>' + data.HighPrice + '</p></dd>' + '<dd><h5>最低</h5><p>' + data.LowPrice + '</p></dd>' + '</dl>' + '</div>';
        return str;
    }
    ...
    function priceListRender(data) {
        var str = '';
        var calcVal = calc.calculateUpDown(data);
        str += '<li data-code="' + data.Code + '">' + '<a href="/marketprice/chart/' + data.MarketId + '" target="_blank">' + '<div class="inside clearfix"><div class="f-l"><div class="name"><h2>' + data.Cn_Name + '</h2><p>' + data.En_Name + '</p></div>' + '<div class="data-area">' + '<div class="total ' + priceStatus(calcVal.priceExchange) + '">' + '<div class="big"><em>' + data.CurrentPrice + '</em>' + '</div><div class="small"><i></i><em>' + calcVal.priceExchange + '</em><em>(' + calcVal.priceLimit + ')</em></div></div>' + '<dl class="param">' + '<dd><h5>昨收</h5><p>' + data.ClosePrice + '</p></dd>' + '<dd><h5>今开</h5><p>' + data.OpenPrice + '</p></dd>' + '<dd><h5>最高</h5><p>' + data.HighPrice + '</p></dd>' + '<dd><h5>最低</h5><p>' + data.LowPrice + '</p></dd>' + '</dl></div>' + '</div>' + '<div class="chart" id="chartLine' + data.MarketId + '">' + '</div>' + '</div></a></li>';
        return str;
    }
    ...
    function indicesListRender(data) {
        var str = '';
        var calcVal = calc.calculateUpDown(data);
        str += '<li data-code="' + data.Code + '">' + '<a href="/marketprice/chart/' + data.MarketId + '" target="_blank">' + '<div class="inside clearfix"><div class="f-l"><div class="name"><h2>' + data.Cn_Name + '</h2><p>' + data.En_Name + '</p></div>' + '<div class="data-area">' + '<div class="total ' + priceStatus(calcVal.priceExchange) + '">' + '<div class="big"><em>' + (data.CurrentPrice == '0' ? '0.000' : data.CurrentPrice) + '</em>' + '</div><div class="small"><i></i><em>' + (calcVal.priceExchange ? calcVal.priceExchange : '0.00%') + '</em><em>(' + (calcVal.priceLimit ? calcVal.priceLimit : '0.000') + ')</em></div></div>' + '<dl class="param">' + '<dd><h5>最高</h5><p>' + (data.HighPrice == '0' ? '0.000' : data.HighPrice) + '</p></dd>' + '<dd><h5>最低</h5><p>' + (data.LowPrice == '0' ? '0.000' : data.LowPrice) + '</p></dd>' + '</dl></div>' + '</div>' + '<div class="chart" id="chartLine' + data.MarketId + '">' + '</div>' + '</div></a></li>';
        return str;
    }
    ...
    function bitListRender(data) {
        var str = '';
        var calcVal = calc.calculateUpDown(data);
        str += '<li data-code="' + data.Code + '" class="bit-list">' + '<a href="/marketprice/chart/' + data.MarketId + '" target="_blank">' + '<div class="inside clearfix"><div class="f-l"><div class="name"><h2>' + data.Cn_Name + '</h2><p>' + data.En_Name + '</p></div>' + '<p class="exchange">' + data.Exchange + '</p>' + '<div class="data-area">' + '<div class="total ' + priceStatus(calcVal.priceExchange) + '">' + '<div class="big"><em>' + (data.CurrentPrice == '0' ? '0.000' : data.CurrentPrice) + '</em>' + '</div><div class="small"><i></i><em>' + (calcVal.priceExchange ? calcVal.priceExchange : '0.00%') + '</em><em>(' + (calcVal.priceLimit ? calcVal.priceLimit : '0.000') + ')</em></div></div>' + '<dl class="param">' + '<dd><h5>最高</h5><p>' + data.HighPrice + '</p></dd>' + '<dd><h5>最低</h5><p>' + data.LowPrice + '</p></dd>' + '</dl></div>' + '</div>' + '<div class="chart" id="chartLine' + data.MarketId + '">' + '</div>' + '</div></a></li>';
        return str;
    }
