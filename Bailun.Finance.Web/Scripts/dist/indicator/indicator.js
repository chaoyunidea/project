(function () {
    ...
    $('.banner dd').on('click', function () {
        ...
        if (!$(this).hasClass('active')) {
            $(this).addClass('active').siblings().removeClass('active').parents('.content').find('ul[data-index="' + dataId + '"]').addClass('active').siblings().removeClass('active');
        }
    });
    //请求banner列表数据
    ...
    function renderComparePop(id) {
        var str = '';
        str += '<div class="mask"><div class="compare-pop" data-id="' + id + '">' + '<div class="title clearfix">' + '<h1>全球经济-' + indicatorNameArr[id - 1] + '对比</h1>' + '<div class="chart-control">' + '<span class="line active" data-type="line"></span><span class="column" data-type="column"></span></div></div>' + '<div class="chart-area">' + '<div class="time-select clearfix">' + '<div class="date">' + '<span class="choice-theme">选择日期：</span>' + '<input class="choice-date" type="text" placeholder="起始日期" id="start-time" value="' + Highcharts.dateFormat('%Y/%m/%d', new Date().setFullYear(new Date().getFullYear() - 5)) + '" readonly>' + '<span class="need-to">至</span>' + '<input class="choice-last" type="text" placeholder="结束日期" id="end-time" value="' + Highcharts.dateFormat('%Y/%m/%d', new Date()) + '" readonly>' + '<a href="javascript:;" class="sure" id="select-data">确定</a>' + '</div>' + '<ul class="year">' + '<li data-year="1">1年</li>' + '<li data-year="5" class="active">5年</li>' + '<li data-year="10">10年</li>' + '<li data-year="0">全部</li>' + '</ul>' + '</div>' + '<div class="chart" id="compare-chart">' + '</div>' + '<div class="compare">' + '<div class="item clearfix">' + '<h3>添加数据对比</h3>' + '</div>' + '<div class="compare-select select-box">' + '<div class="country select">' + '<span class="pick clearfix"><em>选择国家</em><i class="triangle"></i></span>' + '<dl>' + '</dl>' + '</div>' + '<span class="button">确定</span>' + '</div>' + '</div></div><div class="close"><img src="/Images/public/x.png" /></div></div></div>';
        $('body').append(str);
        renderCountryList();
    }
