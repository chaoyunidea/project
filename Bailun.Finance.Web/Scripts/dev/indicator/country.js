(function(){
    ...
    function renderBannerList(data) {
        for (var i = 0; i < data.length;i++) {
            ...
            str += '<li class="' + (chartData[0] > chartData[1] ? 'up' : 'down') + '">'
                + '<a href="/indicator/detail/' + data[i].id + '" target="_blank">'
                + '<h5>' + data[i].name + '</h5>'
                + '<h3>' + chartData[0] + '</h3>'
                + '<div class="chart" id="chartLine' + i + '" style="width:155px;height:40px;">'
                + '</div></a></li>';
