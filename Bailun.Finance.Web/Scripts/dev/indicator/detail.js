(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.code == 0) {
                    ...
                    obj.parents('.country').siblings('.indicator').find('dl').html(renderIndicatorList(dataMain))
                        .end().find('.pick em').html('选择指标');
                }
            }
        })
    }
    ...
    function manageChartData(dataMain) {
        ...
        dataMain.length > 1 && (dateVal = dateVal.unique().sort(function (a, b) { return a - b; }));
        //dateArr.reverse();        
    }
